      ...
      <el-card shadow="always" class="bot-card" style="user-select: none">
        <template #header>
          <div class="card-header">
            ...
            <el-button
              v-if="userId === $store.state.user.id && !isExpand"
              type="warning"
              class="button"
            ...
            <!-- 搜索框 -->
            <el-input
              style="margin-right: 10px; float: right; width: 180px"
              v-model="search"
              clearable
        ...
        >
          ...
          <el-table-column fixed="right" label="Operations">
            <template #default="scope">
              <el-button @click="openEditBot(scope.row)" color="#626aef"
                >修改</el-button
              >
              <el-button type="danger" @click="openDeleteBot(scope.row)"
                >删除</el-button
              >
            </template>
          </el-table-column>
        ...
        >
          ...
          <template #footer>
            <span class="dialog-footer">
              <el-button
                type="warning"
                @click="debugCode(currentOpBot.language, currentOpBot.content)"
                :loading="submitCoding"
              >
                调试
              </el-button>
...
export default {
  ...
  setup(props) {
    ...
    const refresh_bots = () => {
      $.ajax({
        url:
          "https://aigame.zzqahm.top/backend/player/bot/getlist/" +
          props.userId,
        type: "get",
        headers: {
    ...
    const expandBot = () => {
      ElMessageBox.alert(
        "此支付基于支付宝沙箱支付，我们常用的支付宝不能支付，因此提供测试账号：llglqt1652@sandbox.com，密码：111111，支付密码：111111",
        "提示",
        {
