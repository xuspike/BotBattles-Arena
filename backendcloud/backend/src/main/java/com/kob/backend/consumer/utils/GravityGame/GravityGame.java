package com.kob.backend.consumer.utils.GravityGame;
...
public class GravityGame extends Thread{
    ...
    public GravityGame(Integer idA, Bot botA, Integer idB, Bot botB) {
        this.g = new int[7][7];
    ...
    private boolean is_four(int sx, int sy, int color,int direction) {
        int x = sx, y = sy;
        int count = 1;
        for(int i = 0; i < 3; i ++) {
            x += dx[direction];
            y += dy[direction];
            if(x < 0 || x > 6 || y < 0 || y > 6 || g[x][y] != color)
                break;
            count ++;
        }
        x = sx; y = sy;
    ...
    private void judge_result() { // 判断游戏输赢
        if(operator == 1) { // operator改变，此时operator = 1时，玩家A落子
            int flag = (nextStepA % 7 == 0) ? -1 : 0;
            int y = nextStepA / 7 + flag, x = (nextStepA - 1) % 7;
            for(int i = 0; i < 4; i ++)
                if(is_four(x, y, 0, i)) {
                    loser = "B";
                    status = "finished";
            ...
            int flag = (nextStepB % 7 == 0) ? -1 : 0;
            int y = nextStepB / 7 + flag, x = (nextStepB - 1) % 7;
            for(int i = 0; i < 4; i ++)
                if(is_four(x, y, 1, i)) {
                    loser = "A";
                    status = "finished";
    ...
    private void saveToDatabase() { // 将游戏对局存入数据库
        ...
        GravityRecord record = new GravityRecord(
                null,
                PlayerA.getId(),
                PlayerB.getId(),
